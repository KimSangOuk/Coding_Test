# 풀이시간 30분/50분 시간제한 2초 메모리제한 128MB
# 2회차 풀이
# 좌표의 최대 값이 10억이고 이 값이 최대가 되는 중 만약 거리를 이중 구해야한다면 최대 데이터의 크기가 10억인 셈이므로 O(logN)을 이용해야 통과한다. 그렇기 때문에 이진탐색을 생각해 볼 수 있는데, 이 때, 처음 집부터 공유기를 설치했을 때, 구해진 거리로 설치되는 갯수에 따라 이분할 탐색을 해간다면 답을 도출해낼 수 있다.
...
print(binary_search(array,c,0,array[-1]-array[0]))
