...
# 3회차 풀이 - 오답
# 모든 DP를 채워야한다는 사실을 깜빡하고 DP를 중간에 조건이 걸리지 않는 경우를 생각하지 못했다. 왜 생각하지 못했는가? 시간이 초과하는 경우가 끝에 밖에 없다고 생각하고 중간에 오는 경우는 생각하지 못했다. 왜 생각하지 못했는가? 조건문이 걸릴 때, dp가 다 안채워진다는 사실을 간과했다. 반복문에 따른 조건문에 따라 dp가 안들어갈 수 있다는 것을 처음 경험해 보았고 그로 인해 틀린 적도 처음이라 당연히 들어간다는 관념을 가지고 있었다. 다음부터는 조건문에 dp가 걸려있다면 안들어갈 수 있는 경우가 있고 그 경우가 이미 있는 기존의 배열로 처리 되지 않는다면 문제가 있다고 생각해야 할 것이다.
