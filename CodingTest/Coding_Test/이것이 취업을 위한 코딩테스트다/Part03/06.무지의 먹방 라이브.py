# 풀이시간 25분/30분 시간제한 1초 메모리제한 128MB
# 4회차 정답
# 각 음식이 시간에 따라 순차적으로 먹어질 때, 계속해서 앞에서부터 반복해서 먹으면서 k초 먹었을 때, 그 다음 어떤 음식을 먹을것인지 구하는 문제이다. 이 때, 우리는 먼저 처음부터 하나씩 먹는 방법을 생각해 볼 수 있을 때, 시간복잡도를 확인하면 각 음식의 갯수가 200,000개 이고 음식당 양이 억이 넘어가기 때문에 순차적으로 먹는것은 불가능하다는 것을 알 수 있다. 그러면 음식의 번호와 양을 나열해 봤을 때, 시간에 따라 먼저 없어지는 순으로 먹을 수 있다고 생각할 수 있는데 이 때, 그리디 알고리즘이라고 유추해볼 수 있다. 먼저 음식의 총량보다 시간이 많거나 같다면 남은 음식이 없기 때문에 -1이다. 그리고 음식을 먹을 때, 가장 적은 음식부터 사라지기 때문에 가장 적은 음식부터 필요하기 때문에 정렬을 또 생각해 볼 수 있다. 음식의 수도 200,000개 이기 때문에 충분히 가능하다. 음식을 정렬후 꺼낼 거기 때문에 먹은 순대로 deque보다는 heapq를 이용하기로 했다. 현재 먹을 음식의 양, 즉 남아있는 음식 중 가장 적은 음식의 양에서 지금까지 먹은 양을 뺀 양의 남아 있는 음식의 수를 곱한 만큼 우리는 시간이 소모 되므로 이 것이 작을 때 동작을 수행한다. 시간은 먹은 걸린 시간만큼 줄어들고 현재까지 먹은 음식의 양은 따로 기록해 둔다. 그리고 heapq에서 빼면 된다. 그런 후 남아있는 음식 중에서 남은 k초 만큼 진행 시키면 되는데 그러기 위해 번호 순으로 정렬 후 k가 음식의 남은 수보다 많을 수 있기 때문에 길이로 나눈 나머지 만큼 진행해주면 답이 나옴다.
...
def solution(food_times, k):
    ...
    while (q[0][0]-eat_volume)*len(q)<=k:
        k-=(q[0][0]-eat_volume)*len(q)
        eat_volume+=q[0][0]-eat_volume
        heapq.heappop(q)
