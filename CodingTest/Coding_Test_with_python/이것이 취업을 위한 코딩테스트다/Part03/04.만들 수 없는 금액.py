...
# 2회차 풀이
# 시간복잡도 상으로는 짠 알고리즘이 가능하다. 최대 시간복잡도가 많이 드는것이 정렬인 NlogN이기 때문에 1,000이기 때문이다.
# 또한 알고리즘을 생각하는 과정을 보았을 때, 우리는 욕심있게, 만들고자 하는 목표치, 즉 답만 처다본다. 맨처음에 가장 작은 양의 정수인 1을 목표로 잡고 그 목표를 만들 수 있는지 보는것이다. 즉, 이번에 보는 코인이 결과 값보다 작거나 같으면 그 코인을 더했을 때의 수치까지를 만들 수 있다. 그렇기에 그 코인을 더하면 목표치를 갱신하는 식으로 푸는 것이다. 그러다가 coin이 목표치보다 커지면 그 사이의 값은 만들 수 없기 때문에, 반복문을 끝내면 된다. 이 때, 목표치는 달성하지 못했으므로 정답이 된다.
n=int(input())
